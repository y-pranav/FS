...
class Solution {
    ...
    public static void backtrack(int n, int l, StringBuilder sb) {
        if (sb.length() == n) {
            idx++;
            if (idx == l) {
                res = sb.toString();
        ...
        for (char ch = 'a'; ch <= 'c'; ch++) {
            if (sb.length() == 0) {
                ...
                if (sb.charAt(sb.length() - 1) == ch) continue;
                else sb.append(ch);
            }
            backtrack(n, l, sb);
    ...
    public static void main(String[] args) {
        ...
        int l = sc.nextInt();
        idx = 0;
        backtrack(n, l, new StringBuilder());
...
// n = 1 -> 3
// n = 2 -> 6 -> ab, ac, ba, bc, ca, cb
// n = 3 -> 12  -> aba, abc, aca, acb, bab, bac, cab, cac
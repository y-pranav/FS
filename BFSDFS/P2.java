...
class Solution {
    public static void dfs(HashMap<String, PriorityQueue<String>> adj, String cur, ArrayList<String> schedule) {
        PriorityQueue<String> pq = adj.getOrDefault(cur, new PriorityQueue<String>());
        while (!pq.isEmpty()) {
            dfs(adj, pq.poll(), schedule);
        } 
        schedule.add(cur);
    }
